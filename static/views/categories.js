...
// add filter to sxtract categories from all accounts

var CategoriesView = AbstractView.extend({
    subviews: {
        'categoriesTable': new Kernel_View_Ui_Tree({
            collection: Electrum.accounts,
            children: 'SubAccounts',
            columns: [
                {header: 'Name', attr: 'Name'},
                {header: 'Description', attr: 'Description'},
                {header: 'Last month', method: 'computeTotal'},
                {header: 'Last Year', method: 'computeTotal'}
            ],
